...
function handleEditClick(e) {
    ...
    const editedPriority = priorityInput.value; // Add this line
    ...
    // Update the priority
    listItem.childNodes[6].textContent = editedPriority;
...
function exportTasksToTxt() {
    ...
    tasks.forEach((task) => {
      const taskTitle = task.childNodes[1].textContent;
      const dueDate = task.querySelector("#task-dueDate").textContent.split(":")[1].trim();
      const priority = task.querySelector("#task-priority").textContent;
  ...
  // Assuming you have an export button with the ID "exportBtn"
